		...
		<table class="table  table-hover table-striped" id="ps_table" width="100%">
			<thead>
			   <tr class="header">
					<th colspan="5">
					<?php 
					$selected_date = $this->uri->segment(2); 
					?>
					<center>
						<select class="form-control" name="date" id="date" style="width:15%;">
							<option value="">Select Date</option>
							<?php foreach($examDates as $date){?>
							<option value="<?php echo $date['date'];?>" <?php if($selected_date == $date['date']){ echo 'selected'; }?> >
