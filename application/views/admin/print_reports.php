  ...
  <table class="table table-bordered table-sm" style="border: 2px solid;">
    ...
    <tbody >
        ...
        ?>
       	<tr>
    		<td colspan="21">
    			<h5>Post  :  <span><?php echo $rd['designation']  ?></span></h5>
    		</td>
		</tr>
		<?php
		$databyregionid = getinstdata($rd['region_id'],$rd['desigation_id']);
		$g_sen_cnt = 0;
        $g_fill_cnt = 0;
        $g_vac_cnt = 0;
		foreach($databyregionid as $dri)
		{
		?>
		<tr>
    		<td colspan="21">
    			<h5>Institute  : <span><?php echo $dri['inst_name'] ?></span></h5> 
    		</td>
	    </tr>
	        <?php 
	        $databyincourseid = getinstcourse($rd['region_id'],$rd['desigation_id'],$dri['inst_id']);
	         $sen_cnt = 0;
            $fill_cnt = 0;
            $vac_cnt = 0;
	        foreach($databyincourseid as $dcs)
	        {
	        ?>
	        <tr>
		    <td class="text-center"><?php echo $dcs['coursename'] ?></td>
